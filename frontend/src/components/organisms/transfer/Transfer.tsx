...
import {
  ...
  StyledButtonShine,
  StyledCoinText,
  StyledContainer, StyledWalletDisplay, SwapContainer
} from "./Transfer.style";
...
const Transfer: React.FC<Props> = ({
  ...
  const [buttonText, setButtonText] = useState<"Deposit" | "Withdraw">(
    "Deposit"
  );
  ...
  const { balanceOf: getCcdTokenBalance } = useCCDContract(
    ccdContext.account,
    ccdContext.isActive
  );
  ...
  const fetchEthTokenBalance = async () => {
    if (selectedToken) {
      const balance = await getEthTokenBalance(
        selectedToken?.eth_token?.decimals
      );
      ...
      <StyledContainer>
        ...
        <FirstRow>
          {chains
            ...
            .map(({ id, name, account, icon, connect, disconnect }, index) => (
          ...
          <MaxGapRow>
            ...
            <DropdownList open={dropdown}>
              {tokens?.tokens?.map((token) => {
                const { name: ethName, contract: ethContract } =
                  token.eth_token;
                const {
                  name: ccdName,
                  contract_index,
        ...
        >
          <div style={{ position: "relative" }}>
            <Text
              fontSize="16"
              fontColor={transferButtonDisabled ? "White" : "Brown"}
              fontWeight="regular"
            >
              {buttonText}
            </Text>
