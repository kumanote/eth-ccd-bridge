...
const Cornucopia = () => {
  ...
  const [selectedToken, setSelectedToken] =
    useState<Components.Schemas.TokenMapItem>();
  ...
  const [withdrawApproveFee, setWithdrawApproveFee] = useState<
    number | undefined
  >();

  // withdraw estimate ccd state
  const [estimatedWithdrawEnergy, setEstimatedWithdrawEnergy] = useState(0);

  // txs and event id
  const [depositTx, setDepositTx] = useState();
  const [ccdWithdrawTxHash, setCcdWithdrawTxHash] = useState<
    string | undefined
  >();
  const [ccdApproveTxResult, setCcdApproveTxResult] = useState<
    boolean | undefined
  >(false);
  const [ccdApproveTxHash, setCcdApproveTxHash] = useState<
    string | undefined
  >();
  ...
  const [pendingTransaction, setPendingTransaction] =
    useState<Components.Schemas.WalletWithdrawTx>();
  ...
  const { checkAllowance } = useGenerateContract(
    selectedToken?.eth_token?.contract as string, // address or empty string because the address is undefined on first renders
  ...
  const withdrawHandler = async (amount: string) => {
    ...
    const approved = await hasApprove({
      index: selectedToken?.ccd_token.contract_index,
      subindex: selectedToken?.ccd_token.contract_subindex,
    });

    setWithdrawApproved(!!approved);

    if (!approved) {
      const estimatedApprove = await estimateApprove(selectedToken?.ccd_token);
    ...
    if (step?.includes("withdraw")) {
      try {
        ...
        if (withdrawApproved) {
          ...
          const ccdApproveTx = await ccdApprove(
            selectedToken.ccd_token,
            withdrawApproveFee
          );
  ...
  const fetchGas = async (type: "deposit" | "withdraw") => {
    ...
    } else if (type === "withdraw" && pendingTransaction) {
      const erc20PredicateAddress = await typeToVault(); //generate predicate address
      try {
        //try to check the allowance of the token (else you can't estimate gas)
        const tx = await checkAllowance(erc20PredicateAddress);
        if (tx) {
          ...
          const gas = await estimateGas(
            ...
            {
              ccd_token: pendingTransaction.ccd_token,
              eth_token: pendingTransaction.eth_token,
            },
            type,
            merkleProof?.params,
            merkleProof?.proof
  ...
  const initTransactionHandler = (
    ...
    setEventId(transaction.ccd_event_id);
    setCcdWithdrawTxHash(transaction.ccd_tx_hash);
  };

  const pendingTransactionContinueHandler = async (
    transaction: Components.Schemas.WalletWithdrawTx
  ) => {
    ...
    if (merkleProof) {
      ...
      if (tx) {
        const parsedAmount = parseAmount(
          transaction?.amount,
          transaction?.ccd_token?.decimals
        );
        setAmount(parsedAmount.toString());
        setSelectedToken({
          ccd_token: transaction.ccd_token,
          eth_token: transaction.eth_token,
        });
        setStep("progress-withdraw");
        setCcdWithdrawTxHash(transaction?.ccd_tx_hash);
  ...
  useEffect(() => {
    if (depositData) {
      if (depositData?.status === "processed") {
        ...
        setTransferStep(
          parseFloat(
            depositStatusMap.get(depositData?.status + "_number") as string
          )
        );
      } else {
        if (depositStatusMap.get(depositData?.status)) {
          ...
          setTransferStatus(
            depositStatusMap.get(depositData?.status) as string
          ); // set it
          setTransferStep(
            parseFloat(
              depositStatusMap.get(depositData?.status + "_number") as string
          ...
          throw new Error(
            `Deposit failed. Last status: ${depositData?.status}`
          );
        }
      }
    }
  ...
  useEffect(() => {
    if (watchWithdrawData) {
      if (watchWithdrawData?.status === "processed") {
        ...
        setTransferStatus(
          withdrawStatusMap.get(watchWithdrawData?.status) as string
        );
        setTransferStep(
          parseFloat(
            withdrawStatusMap.get(
              watchWithdrawData?.status + "_number"
            ) as string
          )
        );
      } else {
        if (withdrawStatusMap.get(watchWithdrawData?.status)) {
          ...
          setEventId(undefined);
          throw new Error(
            `Withdraw failed. Last status: ${watchWithdrawData?.status}`
          );
        }
      }
    }
  ...
  useEffect(() => {
    ...
    if (
      merkleProof &&
      ccdWithdrawTxHash !== pendingTransaction?.ccd_tx_hash &&
      sessionStorage["CCDSameSession"]
    ) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      setCcdWithdrawTxHash(pendingTransactions[0].ccd_tx_hash);
    }
  }, [pendingTransactions]);
  ...
  if (
    pendingTransaction &&
    sessionStorage["CCDCurrentTx"] !== pendingTransaction.ccd_tx_hash && // this stops the pending transaction component from popping when the same transaction is already finishing
