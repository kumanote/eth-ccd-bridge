...
const useCCDContract = (
  ccdAccount: string | null,
  enabled: boolean
) => {
  ...
  const approve = async (
    ...
    if (!ccdAccount || !enabled) { throw new Error('No account available') };
  ...
  const withdraw = async function (
    ...
    const parsedAmount = parseInt(
      (Number(amount) * 10 ** ccdToken.decimals).toString()
    );
  ...
  const balanceOf = async function (ccdToken?: Components.Schemas.CcdToken): Promise<number> {
    ...
    return ccdToken.decimals !== 18 ? +(balanceOf.toString()) / 10 ** ccdToken.decimals : +ethers.utils.formatEther(balanceOf.toString())
  };
  ...
  const estimateWithdraw = async (
    ...
    if (!res || res.tag === 'failure' || !res.returnValue) {
      throw new Error(
        `RPC call 'invokeContract' on method '${contractNames.bridgeManager}.withdraw' of contract '${bridgeManagerContract.index}' failed with rejectReason ${(res as any)?.reason?.rejectReason}`
      );
    }

    const parsedResult = //energy to ccd
      Number(res?.usedEnergy.toString()) / 10 ** (ccdToken.decimals / 2);
