...
const useRootManagerContract = (
  ...
  const ccdUser = enabled
    ? "0x" +
      Buffer.from(
        Uint8Array.prototype.slice.call(bs58check.decode(ccdAccount || ""), 1)
      ).toString("hex")
    : "";
  ...
  const depositEtherFor = async (amount: string) => {
    ...
    const depositEtherFor = await rootContract.depositEtherFor(
      context.account,
      ccdUser,
      { value: toWei(amount) }
    );
  ...
  const estimateGas = async (
    ...
    if (type === "deposit") {
      if (selectedToken.eth_token.contract === addresses.eth) {
        ...
        gasLimit = (
          await rootContract.estimateGas.depositEtherFor(
            context.account,
            ccdUser,
            { value: toWei(amount) }
          )
        ...
        if (selectedToken.eth_token.decimals === 18) {
          ...
          parsedAmount =
            Number(amount) * 10 ** selectedToken.eth_token.decimals;
        }
        const depositData = ethers.utils.defaultAbiCoder.encode(
          ["uint256"],
          [parsedAmount]
      ...
      gasLimit = (
        await rootContract.estimateGas.withdraw(parsedParams, parts)
      ).toNumber();
    }
